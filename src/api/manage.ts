...
const getAllEmployeeProjects = async (page: number, pageSize: number) => {
  try {
    let baseQuery = supabase.from("employee_projects").select(
      `
            *,
            roles (role_name),
            employees (*),  
...
const createEmployeeProject = async (data: any) => {
  ...
  try {
    ...
    if (roleDetailsError) {
      throw new Error(
        `Failed to fetch role details: ${roleDetailsError.message}`,
      );
    }
    const { data: employeeDetails, error: employeeDetailsError } =
      await supabase
        .from("employees")
        .select("id, name")
        .eq("id", employee_id)
    ...
    if (employeeDetailsError) {
      throw new Error(
        `Failed to fetch employee details: ${employeeDetailsError.message}`,
      );
    }
    const { data: employeeProject, error: employeeProjectError } =
      await supabase
    ...
    if (employeeProjectError) {
      throw new Error(
        `Failed to create employee project: ${employeeProjectError.message}`,
      );
    }
    const result = {
